...
Fracterrain::Fracterrain( unsigned seed /* = 1 */, float jitter /* = 1.0 */, float roughness /* = 0.5 */ )
{
...
void Fracterrain::draw( void )
{
	...
	glTranslatef( -( float )TER_W / 2, 0.0, -( float )TER_W / 2 );
	...
	for ( unsigned x = 0; x + 1 < TER_W; ++x )
	{
		...
		{
			glColor3f( fabs( hmap[x][y] ) / 5.0, x / ( float ) TER_W, y / ( float ) TER_W );
			glVertex3f( x, hmap[x][y], y );

			glColor3f( fabs( hmap[x + 1][y] ) / 5.0, 0.0, 0.0 );
			glVertex3f( x + 1, hmap[x + 1][y], y );
		}
...
void Fracterrain::do_one_square( float jitter, unsigned lX, unsigned lY, unsigned uX, unsigned uY )
{
	int dX = uX - lX,
		dY = uY - lY;
