...
void buildFractal(void) {
	...
	for(int x=0; x<SCREEN_W; ++x)
		for(int y=0; y<SCREEN_H; ++y) {
			...
			pixels[x][y][0] = (char)((totr)*COLOR_CONV);
			pixels[x][y][1] = (char)((totg)*COLOR_CONV);
			pixels[x][y][2] = (char)((totb)*COLOR_CONV);
		}
} // end buildFractal()
