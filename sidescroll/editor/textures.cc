...
void loadTextures(void) {
	...
	fixedWorldObject = new FixedWorldObject[numTextures];
	...
	for(int i = 0; i<numTextures; ++i) {
		...
		fixedWorldObject[i].width         = l->GetImageWidth();
		fixedWorldObject[i].height        = l->GetImageHeight();
		fixedWorldObject[i].textureNumber = i;
