...
void showStatus(void) {
	...
	string tempString = "CurrentFWO:         " + numberToString( *(ui.currentFWO) );
	drawTextBox(tempString, 0.0, 600.0 - CHAR_SIZE - CHAR_LINE_SPACING, 800.0);
	tempString = 	    "Selected Menu Item: " + numberToString(ui.selectedMenuItem);
	drawTextBox(tempString, 0.0, 600.0 - 2 * (CHAR_SIZE + CHAR_LINE_SPACING), 800.0);
	tempString =        "showBackLayer:      ";
	if(ui.showBackLayer) 
		tempString += "true";
	else
		tempString += "false";
	drawTextBox(tempString, 0.0, 600.0 - 3 * (CHAR_SIZE + CHAR_LINE_SPACING), 800.0);
	tempString =        "showFrontLayer:     ";
	if(ui.showFrontLayer) 
		tempString += "true";
	else
		tempString += "false";
	drawTextBox(tempString, 0.0, 600.0 - 4 * (CHAR_SIZE + CHAR_LINE_SPACING), 800.0);
	tempString =        "showCollisions:     ";
	if(ui.showCollisions) 
		tempString += "true";
	else
		tempString += "false";
	drawTextBox(tempString, 0.0, 600.0 - 5 * (CHAR_SIZE + CHAR_LINE_SPACING), 800.0);
	tempString =        "Currently Editing:  ";
	if(ui.backOrFrontFWO == 1) 
		tempString += "front layer";
	else
		tempString += "back layer";
	drawTextBox(tempString, 0.0, 600.0 - 6 * (CHAR_SIZE + CHAR_LINE_SPACING), 800.0);
