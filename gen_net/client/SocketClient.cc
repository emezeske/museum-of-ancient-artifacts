...
bool SocketClient::OpenConnection(const char* serv_addr, int port_no) {
	...
	if(result < 0) {
		printf("Error: couldn't connect to server, errno = %d\n", errno);
		close(socket_no);
		return false;
	}
...
bool SocketClient::SendData(size_t len, void* data) {
	...
	for(int totalBytesSent = 0; totalBytesSent < len;) {
		int bytesSent = write(socket_no, &net_data[totalBytesSent], new_len - totalBytesSent);
