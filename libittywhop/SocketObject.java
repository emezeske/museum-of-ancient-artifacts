...
public class SocketObject extends Thread {
	...
	public static final byte MSG_UNJOIN_GAME = 35; // Join someone else's game 
	...
	public SocketObject(GameObject tgo) {
		...
		catch (IOException e) 
		{
			...
			System.err.println("Server is probably down.");
			System.exit(-1);
		}
		active = true;
		timeSinceLastUpdate = 0;
		// Start the listening thread run()
		start();
	} // end SocketObject()
	...
	private void sendBytes(byte[] bites) {
		...
		{
			System.out.println("Something went awry trying to send some bytes.");
	...
	public void cancelNewGame() {
		if(!active)
			return;
		byte bites[] = { MSG_CANCEL_GAME, (byte)3, (byte)GameObject.playerID }; 
		sendBytes(bites);
	} // end cancelNewGame()
	...
	public void cancelJoinGame() {
		if(!active)
			return;
		byte bites[] = { MSG_UNJOIN_GAME, (byte)4, (byte)GameObject.playerID, (byte)Lobby.gameIWannaJoin }; 
		sendBytes(bites);
	} // end cancelNewGame()
	...
	public void joinGame(int gameID) {
		...
		byte bites[] = { MSG_JOIN_GAME, (byte)4, (byte)GameObject.playerID, (byte)gameID };
		sendBytes(bites);
	} // end joinGame()
	...
	public void sendPositionMessage(byte msgType) {
		...
		int whoAmI = Lobby.playersInLobby[GameObject.playerID].inGameID;
		byte bites[] = { msgType, 
				 (byte)7,
				 (byte)GameObject.playerID, 
	 			 (byte)(GameObject.players[whoAmI].x), 
		 		 (byte)(GameObject.players[whoAmI].y), 
			 	 (byte)GameObject.players[whoAmI].curWallPiece.rotation, 
				 (byte)GameObject.players[whoAmI].curWallPiece.whichPiece };
		sendBytes(bites);
	...
	public void run() {
		...
		{
			// do nothing, will always throw socket closed exception when 
			// shut down
		}
	} // end run()
	...
	private void handleMessage(byte[] msgBuf) {
		...
		{
			if(Lobby.inLobby || Lobby.inLoginScreen)
				return;
		...
		else if(messageType == MSG_GAME_CHAT)
		{
			...
			GameObject.chatDisplay.append(Lobby.playersInLobby[id].name+" says, \""+message+"\"\n");
		}
		else if(messageType == MSG_LOBBY_CHAT)
		{
			...
			int id     = (int)msgBuf[2];
			String message = new String(msgBuf, 3, msgLen - 3);
			Lobby.lobbyChatDisplay.append(Lobby.playersInLobby[id].name+" says, \""+message+"\"\n");
		}
		...
		else if(messageType == MSG_EXIT_LOBBY)
		{
			...
			Lobby.playersInLobby[id].gameIsActive = false;
		...
		else if(messageType == MSG_CANCEL_GAME)
		{
			if(!Lobby.inLobby)
				return;
			int id = (int)msgBuf[2];
			if(Lobby.waitingForGameStart && Lobby.gameIWannaJoin == id)
			{
				Lobby.waitingForGameStart = false;
				Lobby.gameIWannaJoin = -1;
			}
			Lobby.playersInLobby[id].gameIsActive = false;
			Lobby.lobbyChatDisplay.append(Lobby.playersInLobby[id].name+" has cancelled his/her/its game.\n");
		}
		else if(messageType == MSG_UNJOIN_GAME)
		{
			Lobby.lobbyChatDisplay.append(Lobby.playersInLobby[(int)msgBuf[2]].name+" has un-joined "+Lobby.playersInLobby[(int)msgBuf[3]].name+"\'s game.\n");
		}
		else if(messageType == MSG_JOIN_GAME)
		{
			if(!Lobby.inLobby)
				return;
			Lobby.lobbyChatDisplay.append(Lobby.playersInLobby[(int)msgBuf[2]].name+" has joined "+Lobby.playersInLobby[(int)msgBuf[3]].name+"\'s game.\n");

		}
		else if(messageType == MSG_START_GAME)
		{
			if(!Lobby.inLobby || !Lobby.waitingForGameStart)
				return;
