...
public class GameObject extends Applet {
	...
	// Keep a buffer of what the map looks like and only redraw it if
	...
	public void init() {
		...
		map = new byte[WORLD_WIDTH][WORLD_HEIGHT];
		mapChanged = true;

		// Prepare the images
		backBufferImage = createImage(SCREEN_WIDTH, SCREEN_HEIGHT);
		mapImage   = createImage(SCREEN_WIDTH, UI_HEIGHT);
	...
	public void startGame(int numPlayers) {
		...
		Lobby.inLobby = false;
		inGameChatMode = false;
		removeAll(); // layout managers are the devil 
		setLayout(null);
	...
	private void drawWallsAndTerritory(Graphics2D g2d) {
		...
		{
			mapChanged = false;
			Graphics2D mapGraphics = (Graphics2D)mapImage.getGraphics();
			mapGraphics.setRenderingHints(rh);
			mapGraphics.setColor(new Color(0, 225, 0));
			mapGraphics.fillRect(0, 0, SCREEN_WIDTH, UI_HEIGHT);
			for(int x = 0; x < WORLD_WIDTH; ++x)
				for(int y = 0; y < WORLD_HEIGHT; ++y)
				{
					if(map[x][y] == 0)
						continue;
					else if(map[x][y] == PLAYER1_WALL)
						mapGraphics.drawImage(WallPiece.redWallImage, x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, null);
					else if(map[x][y] == PLAYER2_WALL)
						mapGraphics.drawImage(WallPiece.blueWallImage, x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, null);
					else if(map[x][y] == PLAYER3_WALL)
						mapGraphics.drawImage(WallPiece.orangeWallImage, x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, null);
					else if(map[x][y] == PLAYER1_TERR)
					{
						mapGraphics.setColor(PLAYER1_COL);
						mapGraphics.fillRect(x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH);
					}
					else if(map[x][y] == PLAYER2_TERR)
					{
						mapGraphics.setColor(PLAYER2_COL);
						mapGraphics.fillRect(x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH);
					}
					else if(map[x][y] == PLAYER3_TERR)
					{
						mapGraphics.setColor(PLAYER3_COL);
						mapGraphics.fillRect(x * WallPiece.TILE_WIDTH, y * WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH, WallPiece.TILE_WIDTH);
					}
				}
			}
