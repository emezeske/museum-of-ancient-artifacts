...
public class Lobby {
	...
	public static int gameIWannaJoin;
	...
	public void handleLobbyTextEntered() {
		...
		else if(text.startsWith("/join"))
		{
			...
			for(int i = 0; i < LOBBY_CAPACITY; ++i)
				if(playersInLobby[i].gameIsActive && playersInLobby[i].gameName.equals(gameName))
				{
					...
					lobbyChatDisplay.append("Waiting for others to join "+gameName+"\n");
					return;
				}
		...
		else if(text.startsWith("/cancel"))
		{
			if(waitingForGameStart)
			{
				if(playersInLobby[GameObject.playerID].gameIsActive)
				{
					playersInLobby[GameObject.playerID].gameIsActive = false;
					go.socketObject.cancelNewGame();
					lobbyChatDisplay.append("Your game has been cancelled.\n");
					waitingForGameStart = false;
				}
				else if(gameIWannaJoin != -1)
				{
					lobbyChatDisplay.append("Repealing join game action.\n");
					go.socketObject.cancelJoinGame();
					gameIWannaJoin = -1;
					waitingForGameStart = false;
				}
			}
			else
				lobbyChatDisplay.append("Try joining a game or creating one before you /cancel...\n");
