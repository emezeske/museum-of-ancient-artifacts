    ...
    if ( surface )
    {
        ...
        if ( ( surface->w & ( surface->w - 1 ) ) == 0 ) 
        {
            if ( ( surface->h & ( surface->h - 1 ) ) == 0 )
            {
                ...
                if ( nColors == 4 ) // Contains an alpha channel.
                {
                ...
                if ( texture_format )
                {
                    ...
                    if ( SDL_LockSurface( surface ) != -1 )
                    {
                        ...
                        glTexImage2D( GL_TEXTURE_2D, 0, nColors, surface->w, surface->h, 0, texture_format, GL_UNSIGNED_BYTE, surface->pixels );
...
void TextureMap::rasterize( const Vector2Di &position, const Vector2Di &offset ) const
{
    ...
    glPushMatrix();
        glTranslatef( position.x_ + offset.x_, position.y_ + offset.y_, 0.0f );
        glBegin( GL_QUADS );
            vertexLL();
            glVertex2i( 0,             0             );
            vertexUL();
...
void TextureMap::drawDimetric( const Vector2Df &world_position, const float height ) const
{
    rasterize( worldToDimetric( world_position ), Vector2Di( offset_.x_, offset_.y_ + static_cast< int >( roundf( height ) ) ) );
}
