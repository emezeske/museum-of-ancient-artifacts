...
GLdouble sphereX, sphereY, sphereZ;
GLdouble sphereXVel, sphereYVel;
GLfloat angle;
float rotationRate;
...
Point3D getGravity(PlaneEq normal) {

	// X product normal and Gravity vectors, and then X product resulting vector with normal 
	// again to get the force on the object 
	Point3D wop, force, grav;
	grav.x = 0;
	grav.y = 0;
	grav.z = 0.0000051;

	wop.x = normal.y * grav.z - normal.z * grav.y;
	wop.y = normal.z * grav.x - normal.x * grav.z;
	wop.z = normal.x * grav.y - normal.y * grav.x;

	force.x = normal.y * wop.z - normal.z * wop.y;
	force.y = normal.z * wop.x - normal.x * wop.z;
	force.z = normal.x * wop.y - normal.y * wop.x;
...
GLdouble getHeight(PlaneEq normal) {
	...
	GLdouble x = sphereX + (GLdouble)hmWidth / 2 * hmSpacing;
	GLdouble y = sphereY + (GLdouble)hmWidth / 2 * hmSpacing;

	// Determine z (height) when x and y are plugged in and return!
	// normal.z should not ever be 0...
	return -(normal.x * x + normal.y * y + normal.c) / normal.z;
}
...
void display(void) {
	...
	glPushMatrix();
	glRotatef(angle, 0.0, 0.0, 1.0);
	glTranslatef(sphereX, sphereY, sphereZ - /*0.5*/0.21);
	glColor4f(1.0, 0.0, 0.3, 1.0);
	glutSolidSphere(0.21, 13, 13);
...
void idle(void) {
	...
	PlaneEq normal = getNormal(sphereX, sphereY);
	sphereZ = getHeight(normal);
	Point3D grav   = getGravity(normal);
	sphereXVel -= grav.x * delta;
	sphereYVel -= grav.y * delta;
	...
	sphereX += sphereXVel * delta;
	sphereY += sphereYVel * delta;
...
int main(int argc, char **argv) {
	...
	loadTextures();
	angle = 0.0;
	sphereXVel = sphereYVel = sphereX = sphereY = 0;
	lastTime = 0;
	rotationRate = 0;
	...
	for(int x = 0; x < hmWidth; ++x)
		for(int y = 0; y < hmWidth; ++y)
		{
			...
			if(x == 0 || y == 0 || x == hmWidth - 1 || y == hmWidth -1)
				heightMap[x][y] = -10.0;
		}
