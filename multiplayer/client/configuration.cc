...
bool configuration::parse_options( int ac, char **av )
{
    ...
    try
	{
        ...
        config_file.add_options()
            ( "server-address", po::value< string >( &server_address_ )->default_value( "localhost" ), "server hostname or IP address" )
            ( "server-port", po::value< short >( &server_port_ )->default_value( 1500 ), "server port" )
            ;
